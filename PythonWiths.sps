* Python program to generate WITH statements for correlated IVs
* Formatted to work with the MplusPathAnalysis macro
* Written by Jamie DeCoster

* This program accepts a list of names. It then prints a list of lists
...
def PythonWiths(namelist1, namelist2=["empty7663804"], within = 0):
    spss.Submit("title 'PythonWiths start'.")
    if (namelist2==["empty7663804"]):
        for t in range(len(namelist1)):
            for i in range(t+1,len(namelist1)):
                print '["' + namelist1[t] + '", "' + namelist1[i] + '"],'
    else:
        for t in namelist1:
            for i in namelist2:
                print '["' + t + '", "' + i + '"],'
        ...
        if (within == 1):
            ...
            for t in range(len(namelist2)):
                for i in range(t+1,len(namelist2)):
                    print '["' + namelist2[t] + '", "' + namelist2[i] + '"],'
    spss.Submit("title 'PythonWiths end'.")
end program python.
